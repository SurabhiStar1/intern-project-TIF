...
app.post('/create', isAuthenticated, async (req, res) => {
    ...
    try {
        ...
        await db.community.create(data)
            .then(async (data) => {
                let memberData = {
...
app.get("/", async (req, res) => {
    ...
    try {
        let community = await db.community.findAll()
            .catch((err) => {
...
app.get("/me/owner",isAuthenticated, async (req, res) => { //Get My Owned Community
    ...
    try {
        const community = await db.community.findAll({where: {owner : userid}})
...
app.get("/me/member",isAuthenticated, async (req, res) => { //Get My Joined Community
    // List all the data with pagination.
    const userid= req.user.id;
    let role = await db.role.findOne({where:{name:"Community Admin"}})
